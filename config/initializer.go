...
func (i *DynatraceImportInitializer) Initialize(ctx context.Context, mg xpresource.Managed) error {
	...
	case "dynatrace_iam_policy_bindings_v2":
		// Determine scope
		envID := getSpecString(u, "forProvider", "environment")
		if envID == "" {
			envID = getSpecString(u, "initProvider", "environment")
		}

		accID := getSpecString(u, "forProvider", "account")
		if accID == "" {
			accID = getSpecString(u, "initProvider", "account")
		}

		// Retrieve group UUID
		groupUUID := getSpecString(u, "forProvider", "group")
		if groupUUID == "" {
			groupUUID = getSpecString(u, "initProvider", "group")
		}

		// If Group UUID is empty, resolve via GroupRef name
		if groupUUID == "" {
			groupRefName := getSpecString(u, "forProvider", "groupRef", "name")
			if groupRefName == "" {
				groupRefName = getSpecString(u, "initProvider", "groupRef", "name")
			}
			if groupRefName != "" {
				...
				groupKey := client.ObjectKey{
					Name:      groupRefName,
					Namespace: mg.GetNamespace(),
				}
				groupObj := &unstructured.Unstructured{}
				groupObj.SetGroupVersionKind(groupGVK)

				if err := i.kube.Get(ctx, groupKey, groupObj); err != nil {
					return errors.Wrapf(err, "cannot get referenced Group %s in Kubernetes", groupRefName)
				}

				groupNameInDT := getSpecString(groupObj, "forProvider", "name")
				if groupNameInDT == "" {
					groupNameInDT = getSpecString(groupObj, "initProvider", "name")
				}

				if groupNameInDT != "" {
					uuid, err := lookupGroup(ctx, httpClient, accountID, groupNameInDT)
					if err != nil {
						return err
					}
					if uuid == "" {
						return fmt.Errorf("group %s does not exist in Dynatrace yet, retrying lookup later", groupNameInDT)
					}
					groupUUID = uuid
				}
			}
		...
		if groupUUID == "" {
			return errors.New("group reference or UUID not found yet, retrying lookup later")
		}

		// Construct ID
		var externalName string
		if envID != "" {
			externalName = fmt.Sprintf("environment/%s/%s", envID, groupUUID)
		} else if accID != "" {
			externalName = fmt.Sprintf("account/%s/%s", accID, groupUUID)
		} else {
			return errors.New("neither environment nor account scope is configured in PolicyBindingsV2")
		}
