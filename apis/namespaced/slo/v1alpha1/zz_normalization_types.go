// SPDX-FileCopyrightText: 2024 The Crossplane Authors <https://crossplane.io>
//
// SPDX-License-Identifier: Apache-2.0

// Code generated by upjet. DO NOT EDIT.

package v1alpha1

import (
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	"k8s.io/apimachinery/pkg/runtime/schema"

	v1 "github.com/crossplane/crossplane-runtime/v2/apis/common/v1"
	v2 "github.com/crossplane/crossplane-runtime/v2/apis/common/v2"
)

type NormalizationInitParameters struct {

	// (Boolean) When set to true, the error budget left will be shown in percent of the total error budget. For more details see SLO normalization help.
	// When set to true, the error budget left will be shown in percent of the total error budget. For more details see [SLO normalization help](https://dt-url.net/slo-normalize-error-budget).
	Normalize *bool `json:"normalize,omitempty" tf:"normalize,omitempty"`
}

type NormalizationObservation struct {

	// (String) The ID of this resource.
	ID *string `json:"id,omitempty" tf:"id,omitempty"`

	// (Boolean) When set to true, the error budget left will be shown in percent of the total error budget. For more details see SLO normalization help.
	// When set to true, the error budget left will be shown in percent of the total error budget. For more details see [SLO normalization help](https://dt-url.net/slo-normalize-error-budget).
	Normalize *bool `json:"normalize,omitempty" tf:"normalize,omitempty"`
}

type NormalizationParameters struct {

	// (Boolean) When set to true, the error budget left will be shown in percent of the total error budget. For more details see SLO normalization help.
	// When set to true, the error budget left will be shown in percent of the total error budget. For more details see [SLO normalization help](https://dt-url.net/slo-normalize-error-budget).
	// +kubebuilder:validation:Optional
	Normalize *bool `json:"normalize,omitempty" tf:"normalize,omitempty"`
}

// NormalizationSpec defines the desired state of Normalization
type NormalizationSpec struct {
	v2.ManagedResourceSpec `json:",inline"`
	ForProvider            NormalizationParameters `json:"forProvider"`
	// THIS IS A BETA FIELD. It will be honored
	// unless the Management Policies feature flag is disabled.
	// InitProvider holds the same fields as ForProvider, with the exception
	// of Identifier and other resource reference fields. The fields that are
	// in InitProvider are merged into ForProvider when the resource is created.
	// The same fields are also added to the terraform ignore_changes hook, to
	// avoid updating them after creation. This is useful for fields that are
	// required on creation, but we do not desire to update them after creation,
	// for example because of an external controller is managing them, like an
	// autoscaler.
	InitProvider NormalizationInitParameters `json:"initProvider,omitempty"`
}

// NormalizationStatus defines the observed state of Normalization.
type NormalizationStatus struct {
	v1.ResourceStatus `json:",inline"`
	AtProvider        NormalizationObservation `json:"atProvider,omitempty"`
}

// +kubebuilder:object:root=true
// +kubebuilder:subresource:status
// +kubebuilder:storageversion

// Normalization is the Schema for the Normalizations API. The resource
// +kubebuilder:printcolumn:name="SYNCED",type="string",JSONPath=".status.conditions[?(@.type=='Synced')].status"
// +kubebuilder:printcolumn:name="READY",type="string",JSONPath=".status.conditions[?(@.type=='Ready')].status"
// +kubebuilder:printcolumn:name="EXTERNAL-NAME",type="string",JSONPath=".metadata.annotations.crossplane\\.io/external-name"
// +kubebuilder:printcolumn:name="AGE",type="date",JSONPath=".metadata.creationTimestamp"
// +kubebuilder:resource:scope=Namespaced,categories={crossplane,managed,dynatrace}
type Normalization struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.normalize) || (has(self.initProvider) && has(self.initProvider.normalize))",message="spec.forProvider.normalize is a required parameter"
	Spec   NormalizationSpec   `json:"spec"`
	Status NormalizationStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// NormalizationList contains a list of Normalizations
type NormalizationList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []Normalization `json:"items"`
}

// Repository type metadata.
var (
	Normalization_Kind             = "Normalization"
	Normalization_GroupKind        = schema.GroupKind{Group: CRDGroup, Kind: Normalization_Kind}.String()
	Normalization_KindAPIVersion   = Normalization_Kind + "." + CRDGroupVersion.String()
	Normalization_GroupVersionKind = CRDGroupVersion.WithKind(Normalization_Kind)
)

func init() {
	SchemeBuilder.Register(&Normalization{}, &NormalizationList{})
}
